...
public class TransactionContentProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection, @Nullable String[] selectionArg, @Nullable String sortOrder) {
        ...
        switch (uM.match(uri)){
            case ONEROW:
                ...
                squery.appendWhere(TransactionDBOpenHelper.TRANSACTION_ID+"="+idRow);
            default:break;
        }
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues contentValues, @Nullable String s, @Nullable String[] strings) {
        ...
        switch (uriType){
            case ALLROWS:
                rowsUpdated=database.update(TransactionDBOpenHelper.TRANSACTION_TABLE, contentValues,
                        s, strings);
                break;
            ...
            case ONEROW:
                ...
                if(TextUtils.isEmpty(s)){
                    rowsUpdated=database.update(TransactionDBOpenHelper.TRANSACTION_TABLE, contentValues,
                            TransactionDBOpenHelper.TRANSACTION_ID+"="+id, null);
                }
                else{
                    rowsUpdated=database.update(TransactionDBOpenHelper.TRANSACTION_TABLE, contentValues,
                            TransactionDBOpenHelper.TRANSACTION_ID+"="+id + "and "+ s, strings);
                }
                break;
